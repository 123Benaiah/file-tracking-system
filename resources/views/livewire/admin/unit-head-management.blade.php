<div class="py-4 sm:py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Unit Head Management</h1>
                <p class="mt-1 text-gray-600">Assign and manage unit heads</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="Unit, position..."
                           class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500 text-sm">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Unit</label>
                    <select wire:model.live="unitFilter" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500 text-sm">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($unitHeads as $uh)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $uh->unit->name }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $uh->unit->department->name ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $uh->position->title }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $uh->effective_date->format('M d, Y') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $uh->end_date?->format('M d, Y') ?? 'Indefinite' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <button wire:click="toggleStatus({{ $uh->id }})"
                                            wire:loading.attr="disabled"
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium cursor-pointer {{ $uh->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                        {{ $uh->is_active ? 'Active' : 'Inactive' }}
                                    </button>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <button wire:click="edit({{ $uh->id }})"
                                            wire:loading.attr="disabled"
                                            class="text-green-600 hover:text-green-900 mr-3 disabled:opacity-50">
                                        Edit
                                    </button>
                                    <button wire:click="confirmDelete({{ $uh->id }})"
                                            wire:loading.attr="disabled"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                    ...
                                    <p class="mt-2 text-gray-600">No unit heads assigned</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="px-6 py-4 border-t border-gray-200">
                {{ $unitHeads->links(data: ['scrollTo' => false]) }}
            </div>
        </div>
    </div>
                    ...
                    <form wire:submit.prevent="save">
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <h3 class="text-lg font-semibold text-gray-900 mb-4" id="modal-title">
                                {{ $editMode ? 'Edit Unit Head Assignment' : 'Assign Unit Head' }}
                            </h3>
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Unit *</label>
                                    <select wire:model="unit_id" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Assistant Director Position *</label>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">Are you sure you want to delete this unit head assignment?</p>
                                </div>
                            </div>
                        </div>
                    </div>
